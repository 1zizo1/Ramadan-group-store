...
const Products = () => {
  ...
  const formAdd = useForm<FormData>({
    ...
    defaultValues: {
      ...
      description: "",
      price: 0,
      discountPercentage: 0, // Now strictly required as 0
      stock: 0,
      category: "",
      brand: "",
      image: "",
  ...
  const fetchProducts = async (resetPage = false) => {
    ...
    try {
      ...
      const totalCalc = response.data.totalPages || Math.ceil((response.data.total || 0) / perPage);
      setTotalPages(Math.max(Number(totalCalc), 1));
  ...
  const handleEdit = (product: Product) => {
    ...
    formEdit.reset({
      ...
      category: product.category?._id || "", // Added optional chaining
      brand: product.brand?._id || "",       // Added optional chaining
      image: product.image || "",
    });

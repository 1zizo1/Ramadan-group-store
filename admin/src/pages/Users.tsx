...
const UsersPage = () => {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      // Sending page and perPage (limit) to the backend
      const response = await axiosPrivate.get(`/users?page=${page}&limit=${perPage}`);

      if (response?.data) {
        setUsers(response.data.users);
        setTotal(response.data.users.length); // Backend should return the total count of users
        setTotalPages(response.data.totalPages);
        // Backend should return total pages
      }
    } catch (error) {
    ...
    <div className="p-5 space-y-5">
      ...
      <div className="flex items-center justify-between px-2 py-4 border-t bg-gray-50">
        <div className="text-sm text-gray-600">
          ...
          of <span className="font-medium">{total || 0}</span> users
        </div>
        <div className="flex items-center gap-2">
          {/* PREVIOUS BUTTON */}
          <Button
            ...
            // Logic: Disable if on page 1 OR if total pages is 1 or less
            disabled={loading || Number(page) <= 1 || Number(totalPages) <= 1}
          >
            Previous
          </Button>

          <div className="flex items-center gap-1">
            {Array.from({ length: Number(totalPages) || 1 }, (_, i) => i + 1).map((p) => (
              <Button
                key={p}
                variant={Number(page) === p ? "default" : "outline"}
                size="sm"
                className={cn("w-9", Number(page) === p ? "bg-blue-600" : "")}
                onClick={() => setPage(p)}
                disabled={loading}
              >
                {p}
          ...
          <Button
            ...
            // Logic: Disable if on the last page OR if total pages is 1 or less
            disabled={loading || Number(page) >= Number(totalPages) || Number(totalPages) <= 1}
          >
            Next
          </Button>
        </div>

...
const getCategories = asyncHandler(async (req, res) => {
    ...
    const page = parseInt(req.query.page) || 1;  // defualt to page 1
    const perPage = parseInt(req.query.perPage) || 20; // defualt to 10 per page
    const sortOrder = req.query.sort || "asc"; //defualt to be ascending 
    //validate page and perPage
    if (page < 1 || perPage < 1) {
        res.status(400);
        throw new Error("Invalid page or perPage value all must be positive integers");
    }
    // validate sortOrder
...
const createCategory = asyncHandler(async (req, res) => {
    ...
    const category = await Category.create({
        name,
        image: imageUrl || undefined, // Store image url if provided, else unidefined 
        categoryType
    });
    // Response
    if (category) {
